...
Note: We've run out of IPv4 addresses because they were handed out in large blocks. We use black magic (a.k.a. NAT) to keep everything working.
Note: IPv6 address space is larger than visible stars in the universe. Explain double colons for IPv6 address.
...
Note: The 0.0.0.0 part is replaced with the network IP address space.
...
Note: MAC addresses cannot be changed, but can be spoofed on a lot of network hardware.


# Network Models

Note: Encapsulation is key here! Every layer is enveloped by the lower layer.
...
Note: Route selection finds the "best" path, not necessarily the shortest path.
Note: Higher layers (than 3) determine format and further structuring.
Note: Where layer 2 uses MAC addressing, layer 3 uses IP addressing. (lead-in to ARP later)
...
Note: Sender works from top down; receiver works from bottom up.
Note: What kind of processes do you think appear here? Chrome and Word do not live here, but it does include applications that allow Chrome and Word to access networked information.
...
Note: TCP "guarantees" delivery of information.
Note: Explain what stream based means. Note that it is not Netflix.
Note: Why would TCP have increased network overhead? TCP has a 14 header blocks and a 20 byte header
Note: Explain SYN, SYN-ACK, ACK.
...
Note: IP is a connectionless protocol.
Note: For Netflix because it is realtime video streaming.
Note: Why a connectionless (something that doesn't guarantee your message gets there) is good for Netflix? Use frame numbers in explanation.
...
Note: That other network is almost always a wide area network (WAN)
Note: Show 'ip route' output.
...
Note: Most enterprise networks designates a subnet as a "closed circuit" to reduce attack surface of certain systems.
...
Note: For now, just remember that it can translate a human-readable form to an IP address that an application can use to connect to another computer.
